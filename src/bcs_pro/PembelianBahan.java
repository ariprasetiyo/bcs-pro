...
public class PembelianBahan extends javax.swing.JInternalFrame {
        ...
        JComboBoxSatuan.addActionListener(new ActionListener(){
            ...
            public void actionPerformed(ActionEvent e) {
                 ...
                 try{
                     ...
                     int ab = JComboBoxSatuan.getSelectedIndex();
                    JTextFieldPbQty.setText(
                            ConvertAngka.NilaiRupiah(
                            SatuanSelect.ConvertSatuan(JComboBoxSatuan, JTextFieldPbQty,DatahashMap, ab)));
    ...
    private void PrintJasperReport(){
        ...
        AA = JLabelPemberitahuanPenerimaan.getText();
        if (AA.equalsIgnoreCase("Berhasil Di Saved")){
            AA = "-";
        }
        hashMap.put("Judul", AA);
     ...
     */
     ...
     private void ViewPembelianYangSudahDiBuat(){
        ...
        CC =  Dt.GetPembelianPorsi();
        
        if (!CC.equalsIgnoreCase("0")){
           JLabelKetPorsi.setText("Untuk : " + Dt.GetPembelianPorsi() + " " + Dt.GetPembelianStatusSatuan());
        }
    ...
    private void ViewPesanan(){
        ...
        try{
            ...
            if (!PesananJumlahBungkus.equalsIgnoreCase("0")){
                JLabelKetPorsi.setText("Untuk : " + PesananJumlahBungkus + " " + PesananJenisBungkus );
            }
     ...
     private void AmbilDataDatabaseSetelahGetBrowseResep(String PilihData, int Data){
        ...
        ResultSet HQ3       = null;
           ...
           try {
               ...
               if (Data == 1){
                   /*
                    * HQ = stm.executeQuery("SELECT DISTINCT  a.bahan, a.namresep ,sum( a.qty_1_porsi ) a.total_qty, a.satuan, b.porsi_resep "
                           + " from detail_resep a inner join b header_resep on a.namresep = b.namaresep "
                           + " where namresep in ( "+ PilihData + " )"
                           + " GROUP BY bahan");
                           * 
                      SELECT DISTINCT  a.bahan, a.namresep ,sum( a.qty_1_porsi ) total_qty, a.satuan, b.porsi_resep  from detail_resep a inner join  header_resep b on a.namresep = b.nama_resep where a.namresep in ('ayam goreng' ) GROUP BY bahan
                                               * select bahan ,
 if (satuan_1_porsi  = 'ML'  or  satuan_1_porsi  = 'G' and , qty_1_porsi * 11,sum(qty_1_porsi ) ) bos, 
sum(qty_1_porsi )  coba,  qty_1_porsi, satuan_1_porsi  from detail_resep where namresep in ('ayam goreng', 'tempe 2', 'Hanif Resep 1')  group by bahan
                           * 
                           * 
                           * SELECT bahan, if (satuan_1_porsi  = 'ML'  , COUNT(*) ,1) c  FROM detail_resep where namresep in ('ayam goreng', 'tempe 2', 'Hanif Resep 1')   GROUP BY bahan HAVING c > 1;
                      SELECT DISTINCT  a.bahan, a.namresep ,sum( a.qty_1_porsi ) total_qty, a.satuan, b.porsi_resep  from detail_resep a inner join  header_resep b on a.namresep = b.nama_resep where a.namresep in ('ayam goreng' ) GROUP BY bahan
                    
                    */
                   HQ = stm.executeQuery("SELECT DISTINCT  a.bahan, a.namresep"
                           + " ,sum( a.qty_1_porsi ) * c.qty as total_qty, a.satuan, b.porsi_resep, a.satuan_1_porsi "
                           + " from detail_pesanan c right join detail_resep a on c.nama_menu = a.namresep inner join  header_resep b on a.namresep = b.nama_resep "
                           + " where namresep in ( "+ PilihData + " ) and c.trans_no = '" +FF + "' "
                           + " GROUP BY bahan");
                   
                   /*
                    * detail pesanan judul * detail_pesanan
                    */
                   HQ2= stm2.executeQuery("Select porsi as porsi_resep from header_pesanan where trans_no ='" + FF + "'"); 
                   //HQ3= stm3.executeQuery("Select qty as porsi_resep from header_pesanan c inner join detail_pesanan d on c.trans_no = d.trans_no where c.trans_no ='" + FF + "'");  
               }
               else {
               ...
               while(HQ.next()  ){
                   ...
                   if (Data == 1){                      
                       while(HQ2.next()  ){
                           /* 
                            * Sejumlah yang di pesan
                            */
                           Porsi    = HQ2.getString("porsi_resep");
                           
                           //System.err.println("ttt "+Qty + " xzzz ");
                       }
                       //Qty          = HQ.getString("porsi_resep");
                       Qty1Porsi    = HQ.getString("total_qty");
                       Satua1Porsi  = HQ.getString("satuan_1_porsi");
                       
                       /*
                        * Deteksi jika satuan ML atau G
                        *
                       if (Satua1Porsi.equalsIgnoreCase("ML")){
                           //1000000
                           Qty1Porsi = (Double.valueOf(Qty1Porsi).doubleValue() * 0.0000001 )+ "";
                            System.out.println(Satua1Porsi + " xxxxxxxx yyyyyyyyy ");
                       }
                       else if (Satua1Porsi.equalsIgnoreCase("G")){
                           Qty1Porsi = (Double.valueOf(Qty1Porsi).doubleValue() * 0.0001 )+ "";
                            System.out.println(Satua1Porsi + " xxxxxxxx yyyyyyyyyyyyyyy");
                       }
                       System.out.println(Satua1Porsi + " xxxxxxxx ");
                       /*
                        * Rumus pembelian pesanan
                        * Jmlh Pesana  /   Porsi resep * qty 1 porsi
                        */
                       //System.out.println(Bahan + " " + Porsi + " * " + Qty + " * " + Qty1Porsi + " xxxxxxxx ");
                       Qty     = ( Integer.valueOf(Porsi).intValue() )
                               * Double.valueOf(Qty1Porsi).doubleValue() + "";
                   }
                   else if (Data == 0) {
                       while(HQ2.next()  ){
                           Porsi    = HQ2.getString("porsi_resep");
                   ...
                   if (Harga != 0){
                       ...
                       JOptionPane.showMessageDialog(null, "Anda kesalahan item : " + SemBahan +  " " + Bahan);
                   }    
               }
                this.JTabelEdit = true;
           }
    ...
    private void initComponents() {
        ...
        JLabelKetPorsi = new javax.swing.JLabel();
        ...
        JFormattedTextFieldPbUangMuka = new javax.swing.JTextField();
            ...
            .addGroup(JPanelHeader2Layout.createSequentialGroup()
                ...
                .addGroup(JPanelHeader2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(JPanelHeader2Layout.createSequentialGroup()
                        ...
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(JComboBoxPbCaraPesan, javax.swing.GroupLayout.PREFERRED_SIZE, 139, javax.swing.GroupLayout.PREFERRED_SIZE)
                    ...
                    .addGroup(JPanelHeader2Layout.createSequentialGroup()
                        ...
                        .addGroup(JPanelHeader2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addGroup(JPanelHeader2Layout.createSequentialGroup()
                                .addComponent(JTextFieldTransNo, javax.swing.GroupLayout.PREFERRED_SIZE, 134, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addGap(166, 166, 166)
                                .addComponent(JLabelPemberitahuanPb, javax.swing.GroupLayout.PREFERRED_SIZE, 212, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(JLabelKetPorsi, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
        ...
        JPanelPesananHeaderLayout.setVerticalGroup(
            ...
            .addGroup(JPanelPesananHeaderLayout.createSequentialGroup()
                .addContainerGap()
                .addGroup(JPanelPesananHeaderLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addComponent(JPanelHeaderTotal, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
        ...
        jPanel1Layout.setVerticalGroup(
            ...
            .addGroup(jPanel1Layout.createSequentialGroup()
                ...
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addComponent(JPanelPesananHeader, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
        ...
        setBounds((screenSize.width-932)/2, (screenSize.height-444)/2, 932, 444);
    ...
    private javax.swing.JTextField JFormattedTextFieldPbUangMuka;
    private javax.swing.JLabel JLabelKetPorsi;
